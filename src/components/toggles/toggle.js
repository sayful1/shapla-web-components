...
class ShaplaWcToggle extends LitElement {
    ...
    constructor() {
        ...
        this.name = '';
        let parentDataset = this.parentNode.dataset;
    ...
    watchCloseEvent() {
        ToggleUtils.on('close.ShaplaWcToggle', (data) => {
            if (data.togglesUUID === this.parentUUID) {
                let panelBody = this.shadowRoot.querySelector('.shapla-toggle-panel__body');
    ...
    _checked(value) {
        return -1 !== ['yes', 'on', 'true', true, 1, '1'].indexOf(value);
    }

    _panelClass() {
        let classes = ['shapla-toggle-panel', 'shapla-toggle-panel--default']
    ...
    _headingClasses() {
        let classes = ['shapla-toggle-panel__heading'];
        if (this.iconPosition !== 'left') {
            classes.push(`has-icon-${this.iconPosition}`);
        }
        if (-1 !== ['primary', 'secondary'].indexOf(this.titleColor)) {
    ...
    _panelBodyClass() {
        let panelBodyClass = ['shapla-toggle-panel__body'];
    ...
    _iconClass() {
        let classes = ['shapla-toggle-panel__icon', `is-icon-${this.iconPosition}`];
        return classes.join(' ')
    }
