...
import { ShaplaWcPagination } from '../dist/bundle.js'

window.customElements.define('shapla-wc-pagination', ShaplaWcPagination)

describe('ShaplaWcPagination', () => {
  it('has default args totalItems 0, currentPage 1 and perPage 20',
    async () => {
  ...
  it('has end ellipsis if current page less than total pages by upto 4 page',
    async () => {
      const el = await fixture(html`
        <shapla-wc-pagination total-items="200"
                              current-page="6"></shapla-wc-pagination>`)
      expect(el.hasEndEllipsis()).to.equal(true)
    })

  it('throws an event with the expected value when click previous button',
    async () => {
      const el = await fixture(html`
        <shapla-wc-pagination total-items="200"
                              current-page="6"></shapla-wc-pagination>`)
  ...
  it('throws an event with the expected value when click next button',
    async () => {
      const el = await fixture(html`
        <shapla-wc-pagination total-items="200"
                              current-page="6"></shapla-wc-pagination>`)
  ...
  it('can change pre and next text', async () => {
    const el = await fixture(html`
      <shapla-wc-pagination text-previous="&laquo;"
                            text-next="&raquo;"></shapla-wc-pagination>`)
